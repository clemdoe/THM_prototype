...
y, x =np.loadtxt('SingleRod_pressure_axial.txt', delimiter='*', unpack=True)
# y2, x2 =np.loadtxt('Axial_Tcoolant.txt', delimiter='*', unpack=True)


# plotting
plt.plot(x,y, label='BWR SingleRod',color='b', alpha=0.7, linewidth=3)
# plt.plot(x2,y2, label='PWR FA', color='r', linestyle='dashed', linewidth=3)
plt.xlabel('Axial Pressure (Pa)')
